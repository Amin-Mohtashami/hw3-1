...
int recur(int i) {      /* a recursive function */
    ...
    printf("recur call %d: stack@ %lx\n", i, (type_t) &j); /* fix this so it prints the address of j */
    if (i > 0) {
        return recur(i-1);
    }
    return 0;
...
int main (int argc, char *argv[]) {
    ...
    printf("_main  @ %lx\n", (type_t) main); /* fix to print address of main */
    printf("recur @ %lx\n", (type_t) recur); /* fix to print address of recur */
    printf("_main stack: %lx\n", (type_t) &i); /* fix to get address of the stack variable */
    printf("static data: %lx\n", (type_t) &stuff); /* fix to get address of a static variable */
